...
export default function CartProduct({product, index , RemoveSpecificCartItems , UpdateCartProduct}) {
  ...
  const handleInputChange = (e) => {
    const value = Math.max(1, Number(e.target.value) || 1); // Ensure value is at least 1
  ...
  const handleInputBlur = (e) => {
    ...
    UpdateCartProduct(product.product._id, value, setDecrement, setIncrement, product.count);
  };
          ...
          <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
            ...
            <div className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
              <div className="flex items-stretch border-gray-100">
                <Button disabled ={product.count == 1} isLoading={decrement} onChange={handleInputChange} onBlur={handleInputBlur} onPress={()=> UpdateCartProduct(product.product._id , product.count - 1 ,setDecrement,setIncrement ,product.count)}
                ...
                <input
                  ...
                  type="number"
                  value={productCount} onChange={(e) => setProductCount(e.target.value)} onBlur={(e)=> UpdateCartProduct(product.product._id , e.target.value ,setDecrement, setIncrement,product.count)}
                  min="1"
                  
                />
                <Button isLoading={increment} onPress={()=> UpdateCartProduct(product.product._id , product.count + 1 ,setDecrement, setIncrement,product.count)}

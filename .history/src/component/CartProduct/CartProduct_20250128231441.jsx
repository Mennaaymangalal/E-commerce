            ...
            <div className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
              <div className="flex items-stretch border-gray-100">
                <Button isLoading={decrement} onPress={()=> UpdateCartProduct(product.product._id , product.count - 1 ,setDecrement,setIncrement ,product.count)}
                 className="min-w-0 rounded-none ms-3 cursor-pointer rounded-l bg-gray-100 py-1 px-3.5 duration-100  dark:bg-slate-950 hover:bg-blue-500 hover:text-blue-50">
                  -
                </Button>
                <input
                  className=" w-8 border  dark:bg-slate-800 bg-white text-center text-xs outline-none"
                  type="number"
                  value={productCount} onChange={(e) => setProductCount(e.target.value)} onBlur={(e)=>  UpdateCartProduct(product.product._id , product.count + 1 ,setDecrement, setIncrement,product.count)}}
                  min="1"

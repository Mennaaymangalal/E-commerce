...
async function removeProductFromWishlist(productId) {
  ...
  <div className="">
    ...
    >
     <Link to={"/productDetails/" + wishList.id} className=""> <img
        src={product.imageCover}
        alt={product.name}
        className="w-32 h-32 object-cover mb-4 rounded"
      /></Link>
     <Link to={"/productDetails/" + product.id} className=""> <h2 className="text-lg font-semibold line-clamp-1">{wishList.title}</h2></Link>
      ...
      <div className="flex flex-col md:flex-row md:space-x-2 w-full">
       ...
       <button className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 w-full md:w-auto">
        ...
        <button onClick={()=>removeProductFromWishlist(wishList.id)} className="text-red-500 hover:text-red-700 w-full md:w-auto mt-2 md:mt-0">

...
export default function WishList() {
  ...
  const { mutate: removeProduct } = useMutation({
    mutationFn: async (productId) => {
      setRemovingProductId(productId); // ✅ Set loading state for the specific product
    ...
    onSuccess: (productId) => {
      // ✅ Optimistically update the wishlist UI
      queryClient.setQueryData(["wishList"], (oldData) =>
        oldData?.filter((item) => item._id !== productId)
      );
    },
    onSettled: () => {
      setRemovingProductId(null); // ✅ Reset loading state
    },
    onError: (error) => {
